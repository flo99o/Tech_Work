...
router.post("/createad", (req, res) => {
  ...
  const userID = 3;
  connection.query(
    `INSERT INTO Job.offers (job_name, description_position, prerequisite, location, wage, contract, compagny_id, user_id) VALUES ("${content.job_name}", "${content.desc_position}", "${content.prerequisite}", "${content.location}", "${content.wage}", "${content.contract}", "${compagnyID}", "${userID}")`,
    (err, resultat) => {
      if (err) {
        console.log(err);
...
router.post("/compagnyPostOffer", (req, res) => {
  ...
  connection.query(
    `INSERT INTO offers (title, description, release_date, limit_date, recruiter_id) VALUES ("${details.title}","${details.description}","${details.release_date}","${details.limit_date}", "${details.recruiter_id}")`,
    (err, resultat) => {
      if (err) {
        console.log(err);
        res.status(500).send("the Compagny Can't post an offers");
...
router.put("/compagnyUpdateOffer", (req, res) => {
  ...
  console.log(details);
  connection.query(
    `UPDATE offers SET title="${details.title}" description="${details.description}" WHERE id =?"${details.id}"`,
    (err, results) => {
      if (err) {
        console.log(err);
...
router.put("/compagnyUpdateInfo", (req, res) => {
  const details = req.body;
  connection.query(
    `Update compagnies SET title= "${details.title}" description="${details.description}" WHERE id=?`,
    (err, results) => {
      if (err) {
        console.log(err);
...
router.delete("/compagnyDelete/:id", (req, res) => {
  const id = req.params.id;
  connection.query(
    "DELETE FROM Job.compagnies WHERE id=?",
    id,
    (err, results) => {
      if (err) {
        console.log(err);
